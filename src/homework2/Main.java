...
public class Main {
    ...
    private static Vector frameTable = new Vector<Integer>(0); // this should have a cell for each word
    ...
    private static boolean execSysgen(Console console) {
        ...
        frameTable.ensureCapacity(numFreePages);
        ...
        if( processSizeInPages < numFreePages){
            ...
            while(processSizeInPages > 0){
                if(frameTable.elementAt(i) == null){
                    frameTable.add(i,processPCB.getPid());
                    processPCB.addPageLocationToPageTable(i);
                    processSizeInPages--;
                    numFreePages--;
                }
                i++;
    ...
    private static int calculateNumPagesfromProcessSize(int processSize){
        return (int)Math.ceil(processSize/sizeOfPage);
    }
...
//        f = null;
    ...
    private static void showSnapshot(Console console) {
        ...
        if (queueType != 'r' && queueType != 'p' && queueType != 'd' && queueType != 'c') {
            console.printf("Invalid queue type. Must be one of [r,p,d,c] please try again.");
            showSnapshot(console);
        }
        ...
        switch (queueType) {
            case 'r':
                ...
                ListIterator<PCB> readyQ = myBatchVector.listIterator();
                console.printf("\nr:\n");
